  ...
  <div class="bg-slate-700/30 rounded-lg border border-slate-600 p-4 hover:border-slate-500 transition-colors">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex flex-col sm:flex-row gap-2">
        <!-- Bouton Importer dans l'agenda (seulement si pas déjà téléchargé et confirmé) -->
        <button
          v-if="appointment.status === 'confirmed' && !appointment.is_downloaded"
          @click="$emit('download', appointment)"
          :disabled="isDownloading === appointment.id"
          class="bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/50 text-emerald-400 px-4 py-2 rounded-lg font-medium text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          <svg
            v-if="isDownloading === appointment.id"
          ...
          {{ isDownloading === appointment.id ? 'Import...' : 'Importer' }}
        </button>
        
        <!-- Badge "Importé" si déjà téléchargé -->
        <span
          v-if="appointment.status === 'confirmed' && appointment.is_downloaded"
          class="bg-slate-600/30 border border-slate-500/50 text-slate-400 px-4 py-2 rounded-lg font-medium text-sm flex items-center justify-center gap-2"
        >
