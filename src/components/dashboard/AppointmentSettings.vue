          ...
          >
            ...
            <div class="space-y-3 border-t border-slate-700/50 pt-3">
              ...
              <div class="grid grid-cols-2 sm:flex sm:flex-wrap items-end gap-2 sm:gap-3">
                ...
                <!-- Champ de saisie manuelle de l'heure -->
                <div 
                ...
                <!-- Champ de saisie manuelle de la durée -->
                <div 
...
const handleTimeSelectChange = (ruleKey) => {
  ...
  // Si l'utilisateur sélectionne autre chose que "custom", réinitialiser customTime
  if (slot.start !== 'custom') {
    slot.customTime = '';
  } else {
    // Si "custom" est sélectionné, initialiser customTime si vide
    if (!slot.customTime) {
...
const handleDurationSelectChange = (ruleKey) => {
  ...
  // Convertir la valeur en nombre si ce n'est pas "custom"
  if (slot.duration !== 'custom' && typeof slot.duration === 'string') {
    slot.duration = parseInt(slot.duration, 10);
  }
  
  // Si l'utilisateur sélectionne autre chose que "custom", réinitialiser customDuration
  if (slot.duration !== 'custom') {
    slot.customDuration = null;
  } else {
    // Si "custom" est sélectionné, initialiser customDuration si vide
    if (slot.customDuration === null || slot.customDuration === undefined) {
      slot.customDuration = 30; // Valeur par défaut, mais l'utilisateur peut la modifier librement
    }
  }
};

// Gérer la saisie manuelle de la durée
const handleCustomDurationInput = (ruleKey) => {
  const slot = newSlots[ruleKey];
  if (!slot || slot.customDuration === null || slot.customDuration === undefined) return;
  
  // Vérifier que la durée est entre 1 et 480 minutes (8 heures max)
  ...
  // Pas d'arrondi - l'utilisateur choisit librement
};

// Vérifier si un créneau est valide (pour activer/désactiver le bouton)
...
const addSlotToRule = (dayId, ruleId) => {
  ...
  // Si "custom" est sélectionné, utiliser customTime
  ...
  // Si "custom" est sélectionné pour la durée, utiliser customDuration
  ...
  if (slotDuration === 'custom') {
    ...
    // Utiliser la valeur exacte saisie par l'utilisateur (pas d'arrondi)
  ...
  // Vérifier si le créneau existe déjà (même heure de début et même durée)
