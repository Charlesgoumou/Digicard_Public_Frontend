...
/** Clé localStorage : un sceau par commande (employé peut avoir plusieurs commandes). */
export const DEVICE_SEAL_LS_PREFIX = "digicard_device_seal_v1";
...
function writeStoredSeal(orderId, uuid, model) {
  ...
  try {
    localStorage.setItem(
      `${DEVICE_SEAL_LS_PREFIX}_${orderId}`,
      JSON.stringify({ uuid, model, t: Date.now() }),
    );
  } catch {
    /* quota / mode privé */
  }
...
/**
 * Réutilise l’UUID déjà scellé sur cet appareil pour la commande, ou calcule une fois puis enregistre.
 * Garantit une empreinte stable dans le temps (mise à jour navigateur mineure) tout en distinguant deux appareils.
 *
 * @param {number|string|null} orderId - ID commande métier (obligatoire pour le flux employé)
 */
export async function getOrCreateDeviceIdentity(orderId) {
  ...
  writeStoredSeal(orderId, uuid, model);
  return { uuid, model };
}
